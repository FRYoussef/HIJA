...
public class EquityController implements Observer {
    ...
    private void addPlayer(double angleT, int player, String rootPath) {
        try {
            ...
            double x = _eBoard.getRadiusX()*Math.sin(angleT) + _pBoard.getWidth()/2.f - root.getPrefWidth()/1.5f*(Math.sin(angleT)) - (Math.pow(root.getPrefWidth(),1.65f)/96.f);
        	double y = _eBoard.getRadiusY()*Math.cos(angleT) + _pBoard.getHeight()/2.f - root.getPrefHeight()/2.f*(1.f + Math.signum(Math.cos(angleT))*Math.pow(Math.abs(Math.cos(angleT)), 5d)/1.2f)- 10;
            root.setLayoutX(x);
            root.setLayoutY(y);
